...
env = simple_envelope(digital, fs_Hz, ds); % array of our signal's envelope
...
tmpo = abs(max(fft_env)) % results in ~1129 Hz
%% Sync the time
...
time_sync = abs(max(inv_ffts)) % results in 28.821? unlike actual_time_offset_s which is 0.0118
